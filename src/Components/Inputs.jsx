...
export default function Inputs({ setMessages, loading }) {
    const [localInput, setLocalInput] = useState('');

    const addMessage = () => {
        setMessages((prev)=>[...prev, { query: localInput, response: "" }]);
        setLocalInput('');
    }
    ...
    return (
        <div className="flex justify-center align-center py-4 px-1 sm:px-4 sm:my-6 w-full sm:w-9/12 bg-gray-800 text-white sm:rounded-2xl">
            ...
            <input type="text" onChange={(e) => setLocalInput(e.target.value)} value={localInput} onKeyDown={handleKeyDown} id="first_name" className=" text-gray-900 text-sm rounded-md   w-9/12 px-2.5 mx-1 sm:mx-2.5 dark:bg-gray-800  dark:placeholder-gray-400 dark:text-white focus:outline-none" placeholder="Ask your question..." 
